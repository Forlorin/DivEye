...
{
    if (access("diveye.conf", F_OK) == 0)
    {
        Config *config = new Config("diveye.conf");
        return config;
    }
    else
    {
...
connection *initialDatabase(Config *config)
{
    string dbname, user, password, hostaddr, port;
    dbname = config->Read("dbname", dbname);
    user = config->Read("user", user);
    password = config->Read("password", password);
    hostaddr = config->Read("hostaddr", hostaddr);
    port = config->Read("port", port);
    try
    {
        string connectWord = "dbname=" + dbname +
                             ...
                             " hostaddr=" + hostaddr +
                             " port=" + port;
        connection *c = new connection(connectWord);
        if (c->is_open())
        {
